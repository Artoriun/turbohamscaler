...
test('the sitemap lists the public pages and nothing else', async ({ page }) => {
  ...
  // Every public route, in both languages — asserted by name rather than by counting, so
  // adding a page means adding a line here rather than bumping a number that says nothing.
  for (const ending of ['/ja', '/privacy', '/ja/privacy']) {
    expect(
      urls.some((u) => u.endsWith(ending)),
      `sitemap is missing ${ending}: ${urls.join(', ')}`,
    ).toBe(true);
  }
  expect(urls.length, `sitemap urls: ${urls.join(', ')}`).toBe(4);
